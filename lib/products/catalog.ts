...
function withSearch(product: Omit<ProductDocument, "searchText" | "slug">) {
  const slug = slugify(product.title);
  return {
    ...product,
    slug,
    searchText: [product.title, product.description, product.tag, product.category]
      .join(" ")
      .toLowerCase(),
  } satisfies ProductDocument;
}

export const productCatalog: ProductDocument[] = [
  withSearch({
